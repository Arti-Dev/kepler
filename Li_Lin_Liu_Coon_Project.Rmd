...
kepler_clean <- drop_na(kepler_clean)
...
score_vars <- c(
  "koi_disposition",
  "koi_depth", "koi_model_snr", "koi_prad",
  "koi_duration", "koi_steff", "koi_srad",
  "koi_score"
)

score_df <- data[, score_vars]


# Keep only CONFIRMED / FALSE POSITIVE
score_df <- score_df[
  score_df$koi_disposition == "CONFIRMED" |
  score_df$koi_disposition == "FALSE POSITIVE",
]

score_df$koi_disposition <- factor(score_df$koi_disposition)

score_df <- drop_na(score_df)

confirmed_rows <- score_df$koi_disposition == "CONFIRMED"
false_rows <- score_df$koi_disposition == "FALSE POSITIVE"


x <- score_df$koi_score
...
x <- score_df$koi_depth
y <- score_df$koi_score
...
# Transformations
kepler_clean$log_period <- log(kepler_clean$koi_period)
kepler_clean$log_prad <- log(kepler_clean$koi_prad)
kepler_clean$log_srad <- log(kepler_clean$koi_srad)
kepler_clean$log_depth <- log(kepler_clean$koi_depth)
kepler_clean$log_duration <- log(kepler_clean$koi_duration)

# transit depth vs disposition
c("Transit Depth vs Disposition")
transit <- aov(log_depth ~ koi_disposition, data = kepler_clean)
summary(transit)
plot(TukeyHSD(transit))
title("One-way ANOVA for Transit Depth", line = 0.5)
...
summary(snr)
plot(TukeyHSD(snr))
title("One-way ANOVA for Signal-to-Noise Ratio", line = 0.5)


# planet radius vs disposition
c("Planet Radius vs Disposition")
prad <- aov(log_prad ~ koi_disposition, data = kepler_clean)
summary(prad)
plot(TukeyHSD(prad))
title("One-way ANOVA for Planet Radius", line = 0.5)


# orbital period vs disposition
c("Orbital Period vs Disposition")
orbital_period <- aov(log_period ~ koi_disposition, data = kepler_clean)
summary(orbital_period)
plot(TukeyHSD(orbital_period))
title("One-way ANOVA for Orbital Period", line = 0.5)
...
plot(TukeyHSD(stellar_temp))
title("One-way ANOVA for Stellar Temperature", line = 0.5)


# transit duration vs disposition
c("Transit Duration vs Disposition")
transit_duration <- aov(log_duration ~ koi_disposition, data = kepler_clean)
summary(transit_duration)
plot(TukeyHSD(transit_duration))
title("One-way ANOVA for Transit Duration", line = 0.5)
...
title("One-way ANOVA for Impact Parameter", line = 0.5)
...
title("One-way ANOVA for Insolation", line = 0.5)

# equilibrium temperature vs disposition
c("Equilibrium Temperature vs Disposition")
equil_temp <- aov(koi_teq ~ koi_disposition, data = kepler_clean)
summary(equil_temp)
plot(TukeyHSD(equil_temp))
title("One-way ANOVA for Equilibrium Temperature", line = 0.5)


# stellar surface gravity (log(g)) vs disposition
c("Stellar Surface Gravity vs Disposition")
slogg <- aov(koi_slogg ~ koi_disposition, data = kepler_clean)
summary(slogg)
plot(TukeyHSD(slogg))
title("One-way ANOVA for Stellar Surface Gravity", line = 0.5)


# stellar radius vs disposition
c("Stellar Radius vs Disposition")
stellar_rad <- aov(log_srad ~ koi_disposition, data = kepler_clean)
summary(stellar_rad)
plot(TukeyHSD(stellar_rad))
title("One-way ANOVA for Stellar Radius", line = 0.5)


# kepler magnitude flux vs disposition
c("Kepler Magnitude vs Disposition")
kep_mag <- aov(koi_kepmag ~ koi_disposition, data = kepler_clean)
summary(kep_mag)
plot(TukeyHSD(kep_mag))
title("One-way ANOVA for Kepler Magnitude", line = 0.5)
```
...
star_temp <- aov(log_depth ~ koi_disposition * teff_bin, data = kepler_clean)
summary(star_temp)
plot(TukeyHSD(star_temp))
title("Transit Depth by Disposition and Star Temperature", line = 0.5)


# transit duration affected by radius category and disposition
kepler_clean$radius_group <- cut(kepler_clean$log_prad, 
                    breaks = c(-Inf, 1.25, 2, 4, Inf), 
                    labels = c("Earth", "SuperEarth", "SubNeptune",
                               "Neptune+"))
duration_diffs <- aov(log_duration ~ radius_group * koi_disposition, data = kepler_clean)
summary(duration_diffs)
plot(TukeyHSD(duration_diffs))
title("Transit Duration by Disposition and Radius Group", line = 0.5)


# insolation flux affected by disposition and stellar radius bin
kepler_clean$rstar_bin <- cut(kepler_clean$log_srad, breaks = 4)
...
title("Insolation Flux by Disposition and Stellar Radius", line = 0.5)
```
...
### K-means (k=2)
```{r}
kmeans_data <- subset(kepler_clean, select = -c(koi_disposition, teff_bin, radius_group, rstar_bin))
...
plot(boost_fit, i = "koi_model_snr", n.trees = best_iter, main = "Boosting Partial Dependence: Transit SNR (koi_model_snr)")
